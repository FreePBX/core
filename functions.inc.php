...
function core_get_config($engine) {
	...
	switch($engine) {
      ...
      if ($intercom_code != '') {
        if ($has_extension_state) {
          ...
          if ($amp_conf['FORCE_INTERNAL_AUTO_ANSWER_ALL']) {
            $ext->add($mcontext,$exten,'',new ext_gotoif('$["${AMPUSER}"=""|${LEN(${FROM_DID})}|${LEN(${BLINDTRANSFER})}|"${EXTENSION_STATE(${EXTTOCALL})}"!="NOT_INUSE"]','macrodial'));
          } else {
      ...
      */
      ...
      //TODO: don't write this out unless we are running out of the database.
      //
		  core_advancedsettings_write_amportal();//write out amportal.conf
		break;
	}
}

// TODO: make a freepbx_conf method to generate this conf information
//       (add in descriptions also maybe)
//
function core_advancedsettings_write_amportal(){
	global $amp_conf;
	$txt  = "#;--------------------------------------------------------------------------------;\n#;"
	$txt .= _("Do NOT edit this file as it is auto-generated by FreePBX. All modifications to");
	$txt .= " ;\n#; ";
	$txt .= _("this file must be done via the web gui. There are alternative files to make");
	$txt .= "    ;\n#; ";
	$txt .= _("custom modifications, details at:");
	$txt .= "http://freepbx.org/configuration_files       ;\n#;--------------------------------------------------------------------------------;\n\n\n";
	$txt .= "#;--------------------------------------------------------------------------------;\n#; ";
	$txt .= _("All settings can be set from the Advanced Settings page accessible from the General Settings page in FreePBX");
	$txt .= "\n#;--------------------------------------------------------------------------------;\n\n\n\n\n\n\n\n\n\n\n\n";
	foreach ($amp_conf as $key => $val) {
		$txt .= trim($key) . '=' . trim($val) . "\n";
	}
	file_put_contents('/etc/amportal.conf', $txt);
}
