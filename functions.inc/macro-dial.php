...
$ext->add($c,$s,'', new ext_execif('$["${RVOL}"!="" & "${ALERT_INFO}"!=""]', 'Set', 'HASH(__SIPHEADERS,Alert-Info)=${ALERT_INFO}\;volume=${RVOL}'));
$ext->add($c,$s,'', new ext_execif('$["${RVOL}"="" & "${ALERT_INFO}"!="" & "${DB(AMPUSER/${EXTTOCALL}/rvolume)}" != ""]', 'Set', 'HASH(__SIPHEADERS,Alert-Info)=${ALERT_INFO}\;volume=${DB(AMPUSER/${EXTTOCALL}/rvolume)}'));
